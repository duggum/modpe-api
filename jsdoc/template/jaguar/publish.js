...
function addSignatureReturns(f) {
  ...
  f.signature = '(<code class="signature">' + (f.signature || '') + '</code>)';
...
function addSignatureTypes(f) {
  ...
  f.signature = (f.signature || '') + '<span class="type-signature">' + (types.length ?
    ' :' + types.join('|') : '') + '</span>';
}
...
function addAttribs(f) {
  ...
  if (attribs.length) {
    f.attribs = '<span class="type-signature ' + (attribs[0] === 'static' ?
      'static' : '') + '">' + htmlsafe(attribs.length ? attribs.join(',') :
      '') + '</span>';
  }
}
...
function generate(title, docs, filename, resolveLinks) {
  ...
  if (resolveLinks) {
    ...
    html = html.toString().replace(
      /<a\s+([^>]*href\s*=\s*['"]*[^\s'"]*:\/\/)/ig, '<a target="_blank" $1');
  }
...
function generateSourceFiles(sourceFiles) {
  Object.keys(sourceFiles).forEach(function(file) {
    ...
    try {
      source = {
        kind: 'source',
        code: helper.htmlsafe(fs.readFileSync(sourceFiles[file].resolved,
          'utf8'))
      };
    } catch (e) {
      handle(e);
    ...
    generate('Source: ' + sourceFiles[file].shortened, [source],
      sourceOutfile,
      false);
  });
}
...
function attachModuleSymbols(doclets, modules) {
  ...
  return modules.map(function(module) {
    if (symbols[module.longname]) {
      ...
      module.module.name = module.module.name.replace('module:',
        'require("') + '")';
    }
  });
}
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  if (conf['default'].staticFiles) {
    ...
    staticFileFilter = new(require('jsdoc/src/filter')).Filter(conf['default']
      .staticFiles);
    staticFileScanner = new(require('jsdoc/src/scanner')).Scanner();

    staticFilePaths.forEach(function(filePath) {
      var extraStaticFiles = staticFileScanner.scan([filePath], 10,
        staticFileFilter);

      extraStaticFiles.forEach(function(fileName) {
        var sourcePath = fs.statSync(filePath).isDirectory() ?
          filePath :
  ...
  for (var longname in helper.longnameToUrl) {
    if (hasOwnProp.call(helper.longnameToUrl, longname)) {
      ...
      if (myClasses.length) {
        generate('Class: ' + myClasses[0].name, myClasses, helper.longnameToUrl[
          longname]);
      }
      ...
      if (myNamespaces.length) {
        generate('Namespace: ' + myNamespaces[0].name, myNamespaces, helper.longnameToUrl[
          longname]);
      }
      ...
      if (myMixins.length) {
        generate('Mixin: ' + myMixins[0].name, myMixins, helper.longnameToUrl[
          longname]);
      }
      ...
      if (myExternals.length) {
        generate('External: ' + myExternals[0].name, myExternals, helper.longnameToUrl[
          longname]);
      }
    }
  }
